<script>
	...
	let frameworks = $derived(data.frameworks);
	let selectedFrameworkId = $derived(parseInt($page.params.id));

	/** @type {{ open: () => Promise<string | undefined>; }} */
	let modal;

	/** @type {import('@sveltejs/kit').SubmitFunction}*/
	async function confirmDeleteFramework({ cancel }) {
		const returnValue = await modal.open();
	...
	<Card>
		{#snippet header()}
			...
			<div class="flex-1 flex justify-end">
				<button class="btn-primary" onclick={() => goto('/frameworks/new')}>ADD</button>
			</div>
		{/snippet}
		{#snippet body()}
				...
				>
					{#each frameworks as { id, name, description, isPoop } (id)}
						<div
							...
							onkeydown={({ code }) => code === 'Enter' && toggleDetails(id)}
								...
								<Menu>
									<div class="flex flex-col gap-2">
										...
										>
											...
											<button class="btn-warning p-1" onclick={(e) => e.stopPropagation()}
												>DELETE</button
											>
										</form>
									</div>
